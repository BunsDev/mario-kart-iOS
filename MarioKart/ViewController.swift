  ...
  @IBAction func didDoubleTapKart(_ sender: UITapGestureRecognizer) {
    ...
    // YOUR CODE HERE
      ...
      self.translate(kart: sender.view, by: -self.view.frame.width)
          ...
          // you can also pass-in a closure using the regular parameter label syntax
...
//          })
  ...
  private func resetKarts() {
    // YOUR CODE HERE
      UIView.animate(withDuration: 0.5) { // these changes should occur over the duration of 0.4 seconds
  ...
  private func getKartReadyToRace(kart: UIImageView,
    ...
    UIView.animateKeyframes(
      ...
      completion: { _ in
        completion?()//returns so that the code inside the closure in the outter function gets executed
      })
  }
      ...
      getKartReadyToRace(kart: kartView0) { // animate kartView0 first
          self.getKartReadyToRace(kart: self.kartView1) { // ...then kartView1
            self.getKartReadyToRace(kart: self.kartView2) { // ...then kartView2
              ...
              completion?()
                //self.raceKartsWithRandomizedSpeed()// if we call the function here then we don't need to return completion?() for this function to execute above when this runStartingAnimationsOneByOne function is called in the ViewDidAppear
            }
          }
        }
  }
